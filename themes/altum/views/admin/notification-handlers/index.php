...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-bell text-primary-900 mr-2"></i> <?= l('admin_notification_handlers.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/notification-handlers?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/notification-handlers?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
...
<form id="table" action="<?= SITE_URL . 'admin/notification-handlers/bulk' ?>" method="post" role="form">
                ...
                <tr>
                    ...
                    <td class="text-truncate text-muted">
                        <span class="badge badge-light">
                            ...
                            <i class="<?= $available_notification_handlers[$row->type]['icon'] ?> fa-fw fa-sm  mr-1"></i> <?= l('notification_handlers.input.type_' . $row->type) ?>
                        </span>
                    </td>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/notification-handlers/admin_notification_handler_dropdown_button.php', ['id' => $row->notification_handler_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'resource_id' => 'notification_handler_id',
    'has_dynamic_resource_name' => true,
    'path' => 'admin/notification-handlers/delete/'
]), 'modals'); ?>
