...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-user-tag text-primary-900 mr-2"></i> <?= l('admin_team_members.header') ?></h1>
        ...
        <div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/team-members?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/team-members?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
...
<form id="table" action="<?= SITE_URL . 'admin/team-members/bulk' ?>" method="post" role="form">
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.user') ?></th>
                <th><?= l('teams.team') ?></th>
                <th><?= sprintf(l('team_members.input.access'), l('global.all')) ?></th>
            ...
            <?php foreach($data->team_members as $row): ?>
                ...
                <tr>
                    <td data-bulk-table class="d-none">
                        <div class="custom-control custom-checkbox">
                            <input id="selected_team_member_id_<?= $row->team_member_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->team_member_id ?>" />
                            <label class="custom-control-label" for="selected_team_member_id_<?= $row->team_member_id ?>"></label>
                        </div>
                    </td>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex">
                            ...
                            <?php else: ?>
                                <img src="<?= get_gravatar($row->invited_email, 45) ?>" class="rounded-circle mr-3" alt="" />

                                <div class="d-flex flex-column align-self-center">
                                    <span class="text-muted small"><?= $row->invited_email ?></span>
                                </div>
                            <?php endif ?>
                        </div>
                    </td>
                    ...
                    <td class="text-nowrap">
                        <a href="<?= url('admin/team-members?team_id=' . $row->team_id) ?>"><?= $row->name ?></a>
                    </td>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/team-members/admin_team_member_dropdown_button.php', ['id' => $row->team_member_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/partials/universal_delete_modal_url.php', [
    ...
    'path' => 'admin/team-members/delete/'
]), 'modals'); ?>
