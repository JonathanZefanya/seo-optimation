...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-center mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><i class="fas fa-fw fa-xs fa-bolt mr-1"></i> <?= l('audits.header') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('audits.subheader') ?>">
        ...
        <div class="col-12 col-lg-auto d-flex d-print-none">
            ...
            <div class="ml-3">
                <div class="dropdown">
                    <button type="button" class="btn btn-light dropdown-toggle-simple <?= count($data->audits) ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.export') ?>" data-tooltip-hide-on-click>
                        <i class="fas fa-fw fa-sm fa-download"></i>
                    </button>

                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('audits?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('audits?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
            ...
            <div class="ml-3">
                <div class="dropdown">
                    <button type="button" class="btn <?= $data->filters->has_applied_filters ? 'btn-dark' : 'btn-light' ?> filters-button dropdown-toggle-simple <?= count($data->audits) || $data->filters->has_applied_filters ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.filters.header') ?>" data-tooltip-hide-on-click>
            ...
            <div class="ml-3">
                <button id="bulk_enable" type="button" class="btn btn-light <?= count($data->audits) ? null : 'disabled' ?>" data-toggle="tooltip" title="<?= l('global.bulk_actions') ?>"><i class="fas fa-fw fa-sm fa-list"></i></button>
    ...
    <?php if(count($data->audits)): ?>
        <?php if($data->audits_chart): ?>
        <div class="card mb-4">
            <div class="card-body">
                <div class="chart-container <?= !$data->audits_chart['is_empty'] ? null : 'd-none' ?>">
                    <canvas id="audits_chart"></canvas>
                </div>
                <?= !$data->audits_chart['is_empty'] ? null : include_view(THEME_PATH . 'views/partials/no_chart_data.php', ['has_wrapper' => false]); ?>

                <?php if(!$data->audits_chart['is_empty'] && settings()->main->chart_cache ?? 12): ?>
...
<?php require THEME_PATH . 'views/partials/js_chart_defaults.php' ?>
    ...
    <?php ob_start() ?>
        <script>
            if(document.getElementById('audits_chart')) {
                let css = window.getComputedStyle(document.body);
                let audits_color = css.getPropertyValue('--primary');
                let audits_color_gradient = null;

                /* Chart */
                let audits_chart = document.getElementById('audits_chart').getContext('2d');

                /* Colors */
                audits_color_gradient = audits_chart.createLinearGradient(0, 0, 0, 250);
                ...
                new Chart(audits_chart, {
                    type: 'line',
                    data: {
                        labels: <?= $data->audits_chart['labels'] ?? '[]' ?>,
                        datasets: [
                            {
                                label: <?= json_encode(l('audits.audits')) ?>,
                                data: <?= $data->audits_chart['total'] ?? '[]' ?>,
                    ...
                    <?php foreach($data->audits as $row): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap">
                                <div>
                                    <a href="<?= url('audit/' . $row->audit_id) ?>" title="<?= $row->url ?>">
