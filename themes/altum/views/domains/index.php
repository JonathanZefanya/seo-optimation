    ...
    <div class="row mb-4">
        ...
        <div class="col-12 col-lg-auto d-flex d-print-none">
            <div>
                ...
                <?php else: ?>
                    <a href="<?= url('domain-create') ?>" class="btn btn-primary" data-toggle="tooltip" data-html="true" title="<?= get_plan_feature_limit_info($data->total_domains, $this->user->plan_settings->domains_limit, isset($data->filters) ? !$data->filters->has_applied_filters : true) ?>">
                        <i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('domains.create') ?>
                    </a>
                <?php endif ?>
            </div>
            ...
            <div class="ml-3">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('domains?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled' ?>">
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('domains?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled' ?>">
            ...
            <div class="ml-3">
                <div class="dropdown">
                    <button type="button" class="btn <?= $data->filters->has_applied_filters ? 'btn-dark' : 'btn-light' ?> filters-button dropdown-toggle-simple <?= count($data->domains) || $data->filters->has_applied_filters ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.filters.header') ?>" data-tooltip-hide-on-click>
                    ...
                    <?php foreach($data->domains as $row): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap text-muted">
                                <a href="<?= url('websites?domain_id=' . $row->domain_id) ?>" class="mr-2" data-toggle="tooltip" title="<?= l('websites.menu') ?>">
                                    <i class="fas fa-fw fa-pager text-muted"></i>
                                </a>

                                <a href="<?= url('audits?domain_id=' . $row->domain_id) ?>" class="mr-2" data-toggle="tooltip" title="<?= l('audits.menu') ?>">
                                    <i class="fas fa-fw fa-bolt text-muted"></i>
                                </a>

                                <a href="<?= url('archived-audits?domain_id=' . $row->domain_id) ?>" class="mr-2" data-toggle="tooltip" title="<?= l('archived_audits.menu') ?>">
