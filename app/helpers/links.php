...
function remove_url_protocol_from_url($url) {
...
function get_slug($string, $delimiter = '-', $lowercase = true) {
    ...
    $string = preg_replace('/[^\p{L}\d.\-_' . $emojis_regex . ']+/u', $delimiter, $string);

    /* Make sure to remove slashes */
...
function get_domain_from_url($url) {
    ...
    return get_domain_from_host($host);
}
...
function get_domain_from_host($host){
    ...
    if($count === 2){
        ...
        $myhost = get_domain_from_host(explode('.', $myhost, 2)[1]);
    }
    return $myhost;
}
