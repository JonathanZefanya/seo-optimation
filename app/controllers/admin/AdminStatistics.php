...
class AdminStatistics extends Controller {
    ...
    protected function audits() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $audits_chart[$row->formatted_date] = [
                'audits' => $row->total
            ];

            $total['audits'] += $row->total;
        }
        ...
        return [
            'total' => $total,
            'audits_chart' => $audits_chart,
        ];

    }

    protected function archived_audits() {

        $total = ['archived_audits' => 0];
        ...
        while($row = $result->fetch_object()) {
            ...
            $archived_audits_chart[$row->formatted_date] = [
                'archived_audits' => $row->total
            ];

            $total['archived_audits'] += $row->total;
        }
        ...
        return [
            'total' => $total,
            'archived_audits_chart' => $archived_audits_chart,
        ];
